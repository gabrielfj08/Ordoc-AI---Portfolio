...
def debug_create_group():
    ...
    # Simulate POST data:
    data = {
        "name": "Test Group 3", # Use a new name to avoid immediate Duplicate if not intended
    ...
    # Attempt 2: Duplicate
    print("\n--- Attempt 2: Duplicate Group ---")
    serializer2 = UserGroupCreateSerializer(data=data, context=context)
    if serializer2.is_valid():
        try:
            print("Serializer valid (UNEXPECTED). Attempting create...")
            group = serializer2.save()
            print(f"SUCCESS: Created group {group.name} ({group.id})")
        except Exception as e:
            print("\n!!! EXCEPTION CAUGHT DURING SAVE !!!")
            import traceback
        ...
        print(f"Serializer Invalid (EXPECTED): {serializer2.errors}")
