...
import { FilterButtonProcedureTemplateProps } from './types';

const FilterButtonProcedureTemplate = ({
  children,
  params,
  setParams,
}: FilterButtonProcedureTemplateProps) => {
  return (
    <Popover className="flex flex-col w-fit">
      {({ close }) => (
        <Formik
          initialValues={{ status: params.status, source: params.source }}
          onSubmit={(values) => {
            setParams({
              ...params,
              page: 1,
                ...
                <div className="block sm:hidden">
                  <button
                    className={`bg-error h-7 sm:h-9 text-[12px] sm:text-[15px] rounded-r-md px-1 ${
                      params.status || params.source ? 'block' : 'hidden'
                    } 
}`}
                    type="submit"
                    onClick={() => {
...
export default FilterButtonProcedureTemplate;
