...
describe('ProcedureTemplate table cell', () => {
  it('renders the ProcedureTemplateName cell', () => {
    const procedureTemplateCell = render(
      <ProcedureTemplateCell procedure={testProcedure} />
    );

    const { getByText } = procedureTemplateCell;

    expect(getByText('Protocolo')).toBeInTheDocument();
  });
});
