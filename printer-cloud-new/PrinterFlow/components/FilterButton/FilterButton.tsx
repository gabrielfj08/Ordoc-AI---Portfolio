...
import { FlowFilterButtonProps } from './types';

const FlowFilterButton = ({
  children,
  params,
  setParams,
  filterType,
}: FlowFilterButtonProps) => {
  return (
          ...
          onSubmit={(values) => {
            setParams(
              filterType === 'requester'
                ...
                : { ...params, page: 1, status: values.status }
            );
            close();
          }}
        >
              ...
              <div className="flex">
                ...
                >
                  ...
                  <Typography variant="button" color="white">
                    {params.status || params.type ? 'Limpar filtro' : 'Filtrar'}
                  </Typography>
                </Popover.Button>
                <div className="block sm:hidden">
                  <button
                    className={`bg-error h-7 sm:h-9 text-[12px] sm:text-[15px] rounded-r-md px-1 ${
                      params.status || params.type ? 'block' : 'hidden'
                    } 
                }`}
                    type="submit"
                    onClick={() => {
...
export default FlowFilterButton;
