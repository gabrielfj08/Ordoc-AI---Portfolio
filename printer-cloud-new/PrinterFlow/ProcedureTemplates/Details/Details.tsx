...
import { ProcedureTemplateDetailsProps } from './types';
import InactiveProcedureTemplateDetails from './Inactive';

const ProcedureTemplateDetails = ({
  procedureTemplate,
}: ProcedureTemplateDetailsProps) => {
    ...
    <div className="w-screen max-w-full h-screen px-4 sm:px-14 flex flex-col py-8 space-y-4 overflow-auto">
      ...
      <div className={boxClassName}>
        ...
        <div className="space-y-2 w-full">
          ...
          <Typography variant="footnote1" align="center">
            {procedureTemplate.proceduresCount}
          </Typography>
        </div>
      </div>
      ...
      {procedureTemplate.status === 'inactive' && (
        <InactiveProcedureTemplateDetails
          justifiableId={procedureTemplate.id}
        />
      )}
    </div>
  );
...
export default ProcedureTemplateDetails;
