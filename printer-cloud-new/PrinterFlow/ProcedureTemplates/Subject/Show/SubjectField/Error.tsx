...
import { ShowFieldSubjectErrorProps } from './types';
import NewField from '../../../../components/ProcedureTemplates/NewField';

const ShowFieldSubjectError = ({
  procedureTemplate,
}: ShowFieldSubjectErrorProps) => {
  ...
  return (
    <div className="items-center p-4 rounded-lg bg-lighterGray">
      ...
      <div className={hidden}>
        <NewField
          procedureTemplateId={Number(router.query.id)}
          setHidden={setHidden}
        />
      </div>
      {hidden === 'hidden' ? (
        <div className="w-full  bg-white flex items-center space-x-2 justify-center my-4 py-4">
          ...
          <Typography variant="footnote1" color="gray" align="center">
            Erro! Não foi possível carregar a lista de campos do assunto, tente
...
export default ShowFieldSubjectError;
