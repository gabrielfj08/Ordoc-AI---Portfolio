...
import { TaskProcedureFilterButtonProps } from './types';

const TaskProcedureFilterButton = ({
  children,
  params,
  setParams,
}: TaskProcedureFilterButtonProps) => {
  return (
                ...
                <div className="block sm:hidden">
                  <button
                    className={`bg-error h-7 sm:h-9 text-[12px] sm:text-[15px] rounded-r-md px-1 ${
                      params.priority || params.status ? 'block' : 'hidden'
                    }`}
                    type="submit"
                    onClick={() => {
                      formik.resetForm();
...
export default TaskProcedureFilterButton;
