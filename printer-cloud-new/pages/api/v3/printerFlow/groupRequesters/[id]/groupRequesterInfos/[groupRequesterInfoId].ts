...
import { ShowGroupRequesterInfoAPIResponse } from '../../../../../../../services/printer-flow/types';

const transformData = (data): ShowGroupRequesterInfoAPIResponse => {
  return {
    id: data.id,
    status: data.status,
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  return client
    .get(
      `/v3/printer_flow/group_requesters/${req.query.id}/group_requester_infos/${req.query.groupRequesterInfoId}`,
      config
    )
    .then((response) => {
