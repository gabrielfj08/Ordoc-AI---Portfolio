...
const transformData = (data): ShowTaskAPIResponse => {
  return {
    id: data.id,
    deadline: data.deadline,
    priority: data.priority,
    prn: data.prn,
    groupAssigneeId: data.group_assignee_id ? data.group_assignee_id : null,
    procedureId: data.procedure_id,
    name: data.name,
    description: data.description,
    assigneeId: data.assignee_id ? data.assignee_id : null,
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return client
        .get(`/v3/printer_flow/tasks/${req.query.taskId}`, config)
        .then((response) => {
    ...
    case 'PUT':
      return client
        .put(
          `/v3/printer_flow/tasks/${req.query.taskId}`,
    ...
    case 'DELETE':
      return client
        .delete(`/v3/printer_flow/tasks/${req.query.taskId}`, config)
        .then((response) => {
          res.status(response.status).json(transformData(response.data));
        })
        .catch((error) => {
