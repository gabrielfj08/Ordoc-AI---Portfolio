...
const transformDeleteData = (data): DeleteTaskDocumentAPIResponse => {
  return {
    id: data.id,
    status: data.status,
    taskId: data.task_id,
...
const transformShowData = (data): ShowTaskDocumentAPIResponse => {
  return {
    id: data.id,
    status: data.status,
    taskId: data.task_id,
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'DELETE':
      return client
        .delete(
          `/v3/printer_flow/tasks/${req.query.taskId}/task_documents/${req.query.taskDocumentId}`,
          config
        )
        .then((response) => {
