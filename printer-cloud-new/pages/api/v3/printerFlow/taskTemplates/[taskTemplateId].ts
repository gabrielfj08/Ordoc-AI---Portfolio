...
const transformShowData = (data): ShowTaskTemplateAPIResponse => {
  return {
    id: data.id,
    name: data.name,
...
const transformUpdateData = (data): UpdateTaskTemplateAPIResponse => {
  return {
    id: data.id,
    name: data.name,
    description: data.description,
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return client
        .get(
          `/v3/printer_flow/task_templates/${req.query.taskTemplateId}`,
          config
        )
        .then((response) => {
