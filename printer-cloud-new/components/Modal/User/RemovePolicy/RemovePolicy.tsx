import * as React from 'react';
import * as Yup from 'yup';
import { useFormik } from 'formik';
import { queryClient } from '../../../../queryClient';
import { ActionBox, Button, Typography, Checkbox } from 'printer-ui';
import { useAuth, useModal, useSnackbar } from '../../../../hooks';
import { UserService } from '../../../../services';
import { RemovePolicyUserProps } from '../types';

const RemovePolicy = ({
  user_id,
  policy_id,
  policy_name,
}: RemovePolicyUserProps) => {
  const { token, subdomain } = useAuth();
  const { closeModal } = useModal();
  const { showSnackbar } = useSnackbar();

  const formik = useFormik({
    initialValues: {
      checkbox: false,
    },
    validationSchema: Yup.object().shape({
      checkbox: Yup.bool().oneOf(
        [true],
        'Marque a caixa acima para prosseguir'
      ),
    }),
    onSubmit: () => {
      UserService.removePolicy(token, subdomain, user_id, policy_id)
        .then(() => {
          queryClient.invalidateQueries(['users', user_id, 'policies']);
          closeModal();
          showSnackbar(`Permissão removida com sucesso.`, 'success');
          queryClient.invalidateQueries(['users', user_id, 'policies', token]);
          queryClient.invalidateQueries(['policies', policy_id, token]);
        })
        .catch((error) => {
          showSnackbar(error.response.data.message, 'error');
        })
        .finally(() => {
          formik.setSubmitting(false);
        });
    },
  });

  return (
    <ActionBox>
      <form onSubmit={formik.handleSubmit}>
        <ActionBox.Header
          title="Remover permissão do usuário"
          color="error"
          icon="done"
          stroke
          onClose={closeModal}
        />
        <ActionBox.Content>
          <div className="sm:w-[569px] space-y-4">
            <Typography variant="footnote1">
              Você tem certeza da sua ação?
            </Typography>
            <Typography variant="footnote1" className="text-justify">
              Ao clicar em remover permissão, o usuário perderá todos os poderes
              dentro de diretórios, arquivos, acessos e registros desta
              permissão:
            </Typography>
            <Typography variant="footnote1" family="robotoBold" color="blue">
              {policy_name}
            </Typography>
            <span className="flex space-x-2 justify-start items-center pt-2">
              <Checkbox
                id="checkbox"
                name="checkbox"
                onChange={formik.handleChange}
                checked={formik.values.checkbox}
              />
              <label htmlFor="checkbox" className="cursor-pointer">
                <Typography variant="footnote1">
                  Estou ciente de que a permissão será removida do usuário.
                </Typography>
              </label>
            </span>
            {formik.errors.checkbox ? (
              <Typography variant="footnote2" color="error">
                * {formik.errors.checkbox}
              </Typography>
            ) : null}
          </div>
        </ActionBox.Content>
        <ActionBox.Footer>
          <Button type="button" onClick={closeModal} label="Cancelar" />
          <Button
            type="submit"
            color="error"
            label="Remover permissão"
            disabled={formik.isSubmitting}
          />
        </ActionBox.Footer>
      </form>
    </ActionBox>
  );
};

export default RemovePolicy;
