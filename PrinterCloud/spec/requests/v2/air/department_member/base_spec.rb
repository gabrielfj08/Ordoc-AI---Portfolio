...
RSpec.describe 'Base', type: :request do
  ...
  describe 'GET /v2/air/department_member/departments/:id/healthcheck' do
    context 'when user is department member' do
      ...
      it 'responds with status ok' do
        get "/v2/air/department_member/departments/#{department.id}/healthcheck", headers: authorization_headers
    ...
    context "when department's organization is inactive" do
      let(:user) { create(:user) }
      let(:organization) { create(:organization, status: :inactive) }
      let(:departament) { create(:departament, organization: organization) }
      let!(:department_member_role) { create(:role, :department_member, user: user, department: department) }
