    ...
    class TaskDocumentsController < BaseController
      ...
      def index
        ...
        render json: task_documents, meta: { total: task_documents.count },
               each_serializer: V3::TaskDocumentSerializer::List, adapter: :json, status: :ok
      end

      def show
        authorize! :read, @task_document
        render json: @task_document,
               serializer: V3::TaskDocumentSerializer::Show, status: :ok
      end
      ...
      def create
        ...
        render json: task_document, serializer: V3::TaskDocumentSerializer::Show, status: :created
      end
      ...
      def destroy
        ...
        render json: @task_document, serializer: V3::TaskDocumentSerializer::Show, status: :ok
      end

      private

      def create_params
        params.require(:task_document).permit(:s3_key, :name).merge!(created_by_id: current_user.id, source: :upload,
                                                                     key: params['task_document']['s3_key'])
      end
